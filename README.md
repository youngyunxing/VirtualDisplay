...
- 针对常用设备预设分辨率：
  ...
  - 通用 4K UHD / 1080p FHD
- 默认以 HiDPI 模式渲染：菜单中的分辨率为物理输出分辨率，macOS 以一半逻辑分辨率渲染后缩放，画面更清晰
...
2. **左键点击**菜单栏图标，选择要远程的设备预设（如 **OPPO Pad 3**、**MacBook M1 13 寸**）。
   - 开启「单分辨率模式」时，只能同时激活一个预设；选择新预设会自动取消旧预设。
   - 关闭「单分辨率模式」时，可同时激活多个预设；再次点击已选中的预设可取消它。
3. **右键点击**菜单栏图标，打开设置菜单，勾选或取消「单分辨率模式」。
4. 应用会默认以 **HiDPI 模式** 创建虚拟显示器：菜单中的分辨率为物理输出分辨率（远程端实际接收的帧缓冲），macOS 使用一半的逻辑分辨率进行 UI 渲染。例如 **1080p FHD** 物理输出 `1920×1080`、逻辑 `960×540`；**MacBook M1 13 寸缩放** 物理输出 `2880×1800`、逻辑 `1440×900`。
