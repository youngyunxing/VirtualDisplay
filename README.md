...
当前版本：[v5.1.1](../../releases/tag/v5.1.1)
...
发布包是 ad-hoc 签名、未经过 Apple 公证，首次打开可能被 Gatekeeper 拦截，属于正常现象，并不是应用本身有问题。按以下任一方式放行：

1. **右键打开**：在「访达」中右键（或 Control + 点击）`VirtualDisplay.app` →「打开」，确认框里再点「打开」。此后系统会记住选择，正常双击即可。（也可以在「系统设置 → 隐私与安全性」里点「仍要打开」。）
2. **终端移除隔离标记**：
...
> 不推荐用 `sudo spctl --master-disable` 彻底关闭 Gatekeeper。

---

## 关于

点击菜单栏图标底部的「关于」，会展开一个子菜单：
